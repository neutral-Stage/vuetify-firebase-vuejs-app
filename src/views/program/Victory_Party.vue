<template>
  <v-app>
    ...
    >
      <p
        class="flex font-weight-bold white--text"
        style="text-shadow:2px 1px 3px black;text-align: center;font-size: 74px;line-height: 1em;margin: 250px  0 30px 0;letter-spacing: -2px; "
      >{{s.title}}</p>
      ...
      <p
        ...
        style="text-shadow:2px 1px 3px black; text-align: center;font-size: 75px;line-height: 1em;margin: 250px  250px 30px 250px;letter-spacing: -2px;padding:"
      >
...
export default {
  ...
  firestore() {
    ...
    // };
    let cityRef = db.collection("programs").doc("P9q1mL427zlWhjlboKGB");
    let productRef = db.collection("story").doc("ZNsUeasSWuguLhTKOLse");
    let getDoc = cityRef
      .get()
      .then(doc => {
        if (!doc.exists) {
