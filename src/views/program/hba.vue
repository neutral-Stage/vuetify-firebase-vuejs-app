<template>
  <v-app>
    ...
    >
      <v-container style="max-width: 1170px">
        ...
        <v-row justify-center>
          <v-col md="12">
            <div outlined tile style="text-align: center">
              <div
        ...
        <v-row>
          <v-col cols="4" class="small">
            <v-card>
              ...
              <v-card-text
                ...
                v-for="(point, i) in p.firstCol_points"
                :key="i + 'A'"
              >
          ...
          <v-col cols="4" class="small">
            <v-card>
              ...
              <v-card-text
                class="subtitle-1 font-weight-bold"
                v-for="(point, i) in p.secondCol_points"
                :key="i + 'B'"
              >
                <v-icon left small>fiber_manual_record</v-icon>
                {{ point }}
          ...
          <v-col cols="4" class="small">
            <v-card>
              ...
              <v-card-text
                class="subtitle-1 font-weight-bold"
                v-for="(point, i) in p.thirdCol_points"
                :key="i + 'C'"
              >
                <v-icon left small>fiber_manual_record</v-icon>
                {{ point }}
    ...
    <div
      ...
      :key="'help' + index"
    >
      <h1 class="text-center white--text mx-auto small" style="width: 50%">
        {{ p.help }}
      </h1>
      <v-row class="justify-center">
        <v-btn x-large class="title mt-6 small" to="/donate">{{
          p.button
        }}</v-btn>
      </v-row>
    </div>
...
export default {
  ...
  firestore() {
    let cityRef = db.collection("programs").doc("l1w6UPYUtQIBHRgvV571");
    let getDoc = cityRef
      .get()
      .then((doc) => {
        if (!doc.exists) {
          ...
          let productRef = db.collection("story").doc(doc.data().story);
